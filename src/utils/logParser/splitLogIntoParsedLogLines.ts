...
export function splitLogIntoParsedLogLines(logs: string): ParsedSingleLogLine[] {
  // 使用 String.raw 和命名捕获组提高可读性，支持 message 部分包含换行符
  const RE_LOG_LINE = new RegExp(
    String.raw`\[ipid&#LOG!` +
      ...
      String.raw`\)\]` +
      String.raw`(?<message>[\s\S]+?)` + // 捕获消息（支持换行）
      String.raw`\[/ipid&#LOG!` +
  ...
  for (const match of logs.matchAll(RE_LOG_LINE)) {
    const { index: indexStr, funcName, lastNumber: lastNumberStr, stackStr, message } = match.groups!
